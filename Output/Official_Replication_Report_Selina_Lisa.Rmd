...
Accordingly, we started by filtering out the missing values, along with the Independents and Conservatives. Then, we eliminated the states that are represented by only one party, since the graph has to show the within-state differences, not across the states. Next, we took the within-state-party average, in case there is more than one senator who represents a state in a Congress. Finally, as the paper states, we took the within-state differences between parties in each congress and plotted the graph.
```{r filtering and final graph, fig.width=10, fig.height=5}
...
p2
```
'r fig_cap (1, caption = "Our replicated graph of the senate ideal points.)'

![The original graph of the senate ideal points.] (/Users/nhatnguyen/Desktop/Uni/FS20/MA Replication/LisaSelina_Replication/Selina_Lisa_Replication-Paper/Figs) The RStudio logo.


If we compare the replicated with the original graph, there are some noticeable differences.
...
```{r Word Count Add-In, include=FALSE}
...
wordcountaddin::word_count("Official_Replication_Report_Selina_Lisa.Rmd")
```
