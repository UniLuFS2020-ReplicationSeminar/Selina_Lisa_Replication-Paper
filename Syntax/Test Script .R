...
# Check, wether the scores are already normalized
mean(ideals$dynamic_ideal) # ~0
sd(ideals$dynamic_ideal) # 1.6, should be 1
...
#keep only the states represented by both parties
ideals <- ideals %>% 
  filter(!(two_party == F))
